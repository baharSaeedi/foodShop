        ...
        if(!empty($_POST["first_name"]) and !empty($_POST["last_name"]) and !empty($_POST["email"]) and !empty($_POST["mobile"]) and !empty($_POST["password"]) and !empty($_POST["repeatPassword"])  and strlen($_POST["first_name"]) <= 100 and strlen($_POST["last_name"]) <= 100 and filter_var($_POST["email"], FILTER_VALIDATE_EMAIL) and strlen($_POST["password"]) >= 6 and strlen($_POST["repeatPassword"]) >= 6 and $_POST["password"] == $_POST["repeatPassword"] and strlen($_POST["mobile"]) == 11 and substr($_POST["mobile"],0,2) == "09" and is_numeric($_POST["mobile"])) {
            ...
            if(!$response->success){
                ...
                if ($queryExist) {
                    ...
                    if ($queryInsert) {
                        $msgSuccess = true;
                        $msg = "ایمیل فعال سازی حساب کاربری برای شما ارسال خواهد شد.";
...
<div class="col-lg-8 mx-auto">
    <div class="card">
        ...
        <div class="card-body">
            <form novalidate method="post" id="signUpForm" action="signup.php">
                <fieldset>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="first_name" name="first_name" value="<?php echo !empty($_POST["first_name"]) ? (strlen($_POST["first_name"]) <= 100 ? $_POST['first_name'] : '' ) : '' ?>">
                    </div>
                    <div class="form-group">
                        <label for="last_name">نام خانوادگی</label>
                        <input type="text" class="form-control" id="last_name" name="last_name" value="<?php echo !empty($_POST["last_name"]) ? (strlen($_POST["last_name"]) <= 100 ? $_POST['last_name'] : '' ) : '' ?>">
                    </div>
                    <div class="form-group">
                        <label for="mobile">شماره همراه</label>
                        <input type="tel" class="form-control" id="mobile" name="mobile" value="<?php echo !empty($_POST["mobile"]) ? (is_numeric($_POST["mobile"]) ? (strlen($_POST["mobile"]) == 11 ? (substr($_POST["mobile"],0,2) == "09" ? $_POST['mobile'] : '' ) : '') : '' ) : '' ?>">
                    </div>
                    <div class="form-group">
                        <label for="email">ایمیل</label>
                        <input type="email" class="form-control" id="email" name="email" value="<?php echo !empty($_POST["email"]) ? (filter_var($_POST["email"], FILTER_VALIDATE_EMAIL) ? $_POST['email'] : '' ) : '' ?>">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <input name="signUp-btn" type="submit" class="btn btn-dark my-btn" value="ثبت نام"/>
                        <a href="<?php echo DOMAIN; ?>" class="btn btn-secondary">لغو</a>
                    </div>

                    <p>پیش از این عضو شده اید؟<a href="./login.php"> وارد شوید </a></p>
